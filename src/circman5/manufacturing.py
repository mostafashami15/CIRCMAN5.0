...
class AdvancedPVManufacturing:
    def __init__(self):
        ...
        self.batches = pd.DataFrame(columns=self.batch_dtypes.keys()).astype(
            self.batch_dtypes
        )
        self.quality_data = pd.DataFrame(columns=self.quality_dtypes.keys()).astype(
            self.quality_dtypes
        )
        self.circular_metrics = pd.DataFrame(
            columns=self.circular_dtypes.keys()
        ).astype(self.circular_dtypes)
    ...
    def start_batch(self, batch_id: str, stage: str, input_amount: float) -> None:
        ...
        self.batches = pd.concat(
            [self.batches, pd.DataFrame([new_batch])], ignore_index=True
        )
    ...
    def complete_batch(self, batch_id, output_amount, energy_used):
        ...
        if batch_id not in self.batches["batch_id"].values:
            raise ValueError(f"Batch {batch_id} not found!")  # ✅ Raises an exception
    ...
    def record_quality_check(
        ...
        self.quality_data = pd.concat(
            [self.quality_data, pd.DataFrame([new_quality_record])], ignore_index=True
        )
    ...
    def record_circular_metrics(
        self, batch_id, recycled_content, recyclable_output, water_reused
    ):
        ...
        self.circular_metrics = pd.concat(
            [self.circular_metrics, pd.DataFrame([new_circular_record])],
            ignore_index=True,
        )
    ...
    def get_batch_summary(self, batch_id):
        ...
        circular = self.circular_metrics[
            self.circular_metrics["batch_id"] == batch_id
        ].iloc[0]
        ...
        print(
            f"Waste Generated: {batch['input_amount'] - batch['output_amount']:.2f} kg"
        )
        ...
        if batch["energy_used"] > 0:
            ...
            print(
                f"Energy Efficiency: {batch['output_amount']/batch['energy_used']:.2f} kg/kWh"
            )
    ...
    def visualize_batch_performance(self, batch_id, save_path=None):
        ...
        bars = plt.bar(
            ["Input", "Output", "Waste"], values, color=["blue", "green", "red"]
        )
    ...
    def save_data_to_excel(self, filename="pv_manufacturing_data.xlsx"):
        ...
        try:
            ...
            with pd.ExcelWriter(filename) as writer:
                ...
                self.quality_data.to_excel(
                    writer, sheet_name="Quality_Data", index=False
                )
                self.circular_metrics.to_excel(
                    writer, sheet_name="Circular_Metrics", index=False
                )
    ...
    def create_summary_report(self):
        ...
        summary_data = {"Metric": [], "Value": [], "Unit": []}
        ...
        summary_data["Unit"].extend(["kg", "kg", "%", "kWh"])
    ...
    def perform_advanced_analysis(self):
        ...
        analysis_results["Process Efficiency"] = {
            ...
            "energy_efficiency": (
                self.batches["output_amount"] / self.batches["energy_used"]
            ).describe(),
            "waste_rate": (
                (self.batches["input_amount"] - self.batches["output_amount"])
                / self.batches["input_amount"]
            ).describe(),
        }
