        ...
        for _ in range(self.days):
            ...
            for hour in range(8, 16):  # 8 AM to 4 PM shift
                for line in self.production_lines:
                    ...
                    base_output = 100 if line == 'LINE_A' else 90 if line == 'LINE_B' else 85
                    ...
                    data.append({
                        ...
                        'yield_rate': random.uniform(0.92, 0.98)  # typical PV manufacturing yield
                    })
    ...
    def generate_energy_data(self) -> pd.DataFrame:
        ...
        for _ in range(self.days):
            for hour in range(24):
                ...
                base_consumption = 100 if 8 <= hour <= 16 else 50  # Higher during production hours
                
                for line in self.production_lines:
                    data.append({
                        'timestamp': timestamp,
                        'production_line': line,
                        'energy_consumption': base_consumption * random.uniform(0.9, 1.1),
    ...
    def generate_quality_data(self) -> pd.DataFrame:
        ...
        for _ in range(self.days):
            for hour in range(8, 16):
                ...
                for line in self.production_lines:
                    for _ in range(3):
                        base_efficiency = random.uniform(0.20, 0.22)  # Typical PV cell efficiency
                        ...
                        data.append({
                            ...
                            'visual_inspection': random.choice(['Pass', 'Minor_Issues', 'Fail'])
                        })
            current_date += timedelta(days=1)
            ...
            for hour in range(8, 16):
                ...
                for material in self.material_types:
                    ...
                    data.append({
                        ...
                        'recycled_amount': waste_generated * random.uniform(0.7, 0.9),
                        'batch_id': random.choice(self.batch_ids)
                    })
            current_date += timedelta(days=1)
