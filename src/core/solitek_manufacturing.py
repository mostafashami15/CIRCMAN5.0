...
class SoliTekManufacturingAnalysis:
    ...
    def generate_visualization(self, metric_type: str, save_path: Optional[str] = None) -> None:
        """
    ...
    def _calculate_energy_efficiency(self) -> float:
        ...
        if self.energy_data.empty:
            return 0.0
        return (self.energy_data['efficiency_rate'] * 
                self.energy_data['energy_consumption']).mean()
    ...
    def _calculate_waste_metrics(self) -> Dict:
        ...
        return {
            "total_waste": self.material_flow['waste_generated'].sum(),
            "recycling_rate": (self.material_flow['recycled_amount'].sum() / 
                             self.material_flow['waste_generated'].sum() * 100),
            "waste_by_type": self.material_flow.groupby('material_type')['waste_generated'].sum()
        }
