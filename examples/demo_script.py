...
from circman5.config.project_paths import ProjectPaths  # ✅ FIXED IMPORT
...
def main():
    ...
    # ✅ Ensure proper directory setup using ProjectPaths
    run_dir = Path(ProjectPaths.get_run_directory())
    input_data_dir = run_dir / "input_data"
    reports_dir = run_dir / "reports"
    visualizations_dir = run_dir / "visualizations"

    # ✅ Setup logging
    logger = setup_logger("demo_script", log_dir=str(run_dir))
    ...
    try:
        ...
        # ✅ Save generated data to input_data folder
        production_data.to_csv(input_data_dir / "test_production_data.csv", index=False)
        quality_data.to_csv(input_data_dir / "test_quality_data.csv", index=False)
        energy_data.to_csv(input_data_dir / "test_energy_data.csv", index=False)
        material_flow.to_csv(input_data_dir / "test_material_data.csv", index=False)
        ...
        for metric in ["production", "energy", "quality", "sustainability"]:
            ...
            logger.info(f"  Saved {metric} visualization to {viz_path}")
